...
import ExamsTab from "@/components/admin/clinical/ExamsTab";

const AdminRegistroClinicoExames = () => {
  ...
  return (
    <AdminLayout title="Exames Gestacionais" backTo="/admin/registro-clinico">
        ...
        <div className="flex items-center gap-3">
          ...
          {record && (
            <Badge variant="outline" className="font-heading text-xs">
              {record.gestationalExams.length} exame(s)
            </Badge>
          )}
        </div>

        {record ? (
          <ExamsTab record={record} onRecordUpdate={(r) => updateRecord(r.id, r)} />
        ) : (
          <Card className="bg-white/40 backdrop-blur-xl border-white/50 shadow-lg shadow-black/5">
            <CardContent className="p-8 text-center">
              <p className="text-sm text-muted-foreground font-heading">Selecione uma gestante para visualizar os exames</p>
            </CardContent>
          </Card>
        )}
      </div>
...
export default AdminRegistroClinicoExames;
