...
import VaccinesTab from "@/components/admin/clinical/VaccinesTab";

const AdminRegistroClinicoVacinas = () => {
        ...
        <div className="flex items-center gap-3">
          ...
          {record && (
            <Badge variant="outline" className="font-heading text-xs">
              {(record.vaccines || []).length} vacina(s)
            </Badge>
          )}
        </div>

        {record ? (
          <VaccinesTab record={record} onRecordUpdate={(r) => updateRecord(r.id, r)} />
        ) : (
          <Card className="bg-white/40 backdrop-blur-xl border-white/50 shadow-lg shadow-black/5">
            <CardContent className="p-8 text-center">
              <p className="text-sm text-muted-foreground font-heading">Selecione uma gestante para visualizar as vacinas</p>
            </CardContent>
          </Card>
        )}
      </div>
...
export default AdminRegistroClinicoVacinas;
